...
function sumTimeEntriesWFIntoArray(tArray) {
	itemCap = capId;
	if (arguments.length > 1) itemCap = arguments[1];
	// get time for workflow tasks
	var workflowResult = aa.workflow.getTasks(itemCap);
	if (workflowResult.getSuccess()) {
		var wfObj = workflowResult.getOutput();
		for (i in wfObj) {
			var fTask = wfObj[i];
			var tName = "" + fTask.getTaskDescription();
			var sumTime = sumTimeEntriesForTask(tName, null, null, "L", "Y"); // get all locked and billable time entries for this task
			if (IsStrInArry(tName, tArray)) {
				tArray[tName] += parseFloat(sumTime);
			}
			else {
				tArray[tName] = parseFloat(sumTime);
			}
...
function sumTimeEntriesForTask(taskName) { // optional timeGroup, timeType, Locked/UnLocked, Billable/NonBillable, capId
	timeGroup = null;
	if (arguments.length > 1) 
		timeGroup = arguments[1];
	timeType = null
	if (arguments.length > 2)
		timeType = arguments[2];
	lockedUnlocked = null;
	if (arguments.length > 3)
		lockedUnlocked = arguments[3];
	billable = null;
	if (arguments.length > 4)
		billable = arguments[4];
	var itemCap = capId;
	if (arguments.length > 5) itemCap = arguments[5];
	
	var retSum = 0;
	var taArray = getTimeAccountingEntries(taskName, null, itemCap, timeGroup, timeType);
	for (var tIndex in taArray) {
		var timeLogModel = taArray[tIndex];
		if (lockedUnlocked == null || (lockedUnlocked != null && timeLogModel.getTimeLogStatus()==lockedUnlocked)) {
			if (billable == null || (billable != null && timeLogModel.getBillable() == billable)) {
...
function getTimeAccountingEntries(taskName) { 
	...
	//optional params: processID, capID, timeGroup, timeType
	...
	if (arguments.length > 3) {
		if (arguments[3] != null && arguments[3] != "") 
			timeGroup = arguments[3];
	}
	var timeType = null;
	if (arguments.length > 4) {
		if (arguments[4] != null && arguments[4] != "") 
			timeType = arguments[4];
	}
	...
	var workflowResult = aa.workflow.getTasks(itemCap);
 	if (workflowResult.getSuccess()) {
		...
		for (i in wfObj) {
   			var fTask = wfObj[i];
			if (fTask.getTaskDescription().toUpperCase().equals(taskName.toUpperCase())  && (!useProcess || fTask.getProcessCode() == processName)) {
				var entityID = "" + fTask.getStepNumber() + ":" + fTask.getProcessID();
				taResult = aa.timeAccounting.getTimeLogModelByEntity(itemCap, entityID, "WORKFLOW", null, null);
